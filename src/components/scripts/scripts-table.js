...
const ScriptTable = () => {
  ...
  const loadData = () => {
    get_scripts().then(setScripts);
  };

  const generateDeleteCallback = (script) => () => {
    delete_script(script.ID).then(loadData);
  };

  const handleSubmit = (event) => {
    const file = event.target.files[0];
    upload_script(file).then(loadData);
  };
              ...
              <TableRow key={script.ID}>
                <TableCell component="th" scope="row">
                  <Grid container direction="row">
                    <IconButton
                      onClick={generateDeleteCallback(script)}
                      color="inherit"
                    ...
                    <Typography color="rgba(0, 0, 0, .3)" paddingLeft={1}>
                      ID: {script.ID}
                    </Typography>
                  </Grid>
                </TableCell>
