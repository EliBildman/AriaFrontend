...
const ScheduleAccordion = () => {
  ...
  const generateUpdateCallback = (schedule) => () => {
    update_schedule(schedule).then(loadData);
  };
  ...
  const generateDeleteCallback = (schedule) => () => {
    delete_schedule(schedule.ID).then(() => {
      loadData();
      setExpanded(-1);
    });
  };
  ...
  if (routinesLoaded && schedulesLoaded) {
      ...
      >
        <AccordionSummary>
          ...
          <Typography paddingLeft={1}>
            ({cronstrue.toString(schedule.cron_string)})
          </Typography>
          <Typography paddingLeft={1} color="rgba(0, 0, 0, .3)">
            ID: {schedule.ID}
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
    ...
    if (showAddition)
      schedule_rows.push(
        <ScheduleAddition
          key="html is cringe"
          add={addScheduleCallback}
          cancel={() => setShowAddition(false)}
        />
      );
  }
